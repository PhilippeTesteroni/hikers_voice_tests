...
- [Архитектура](#архитектура-)
  ...
  - [Модерация и инвалидация](#модерация-и-инвалидация)
...
- `wait_for_data_update()` — ожидание инвалидации ISR кэша с hard reload
...
- Пересчёт рейтинга
- **ISR кэш инвалидация** (10 секунд на сервере)

#### Инвалидация ISR кэша

**Проблема:** Next.js ISR кэширует страницы. После модерации данные не появляются моментально.

**Решение:** Метод `wait_for_data_update()` с **hard reload**:

```python
async def wait_for_data_update(self, check_fn, timeout=10000, retry_interval=6000):
    while time_left > 0:
        if await check_fn():  # проверка появления данных
            return True
        
        # Hard reload с полной очисткой кэша (CDP)
        ...
        await self.page.wait_for_timeout(retry_interval)
    return False
```

**Когда используется:**
- После модерации отзыва
...
```python
# Проверяем, что отзыв появился на странице компании
success = await company_page.wait_for_data_update(
    check_fn=lambda: company_page.get_reviews_count() == 1,
    timeout=30000,
    retry_interval=6000
)
assert success, "Review did not appear after moderation"
```
...
- ISR кэш: больше не нужны retry благодаря `wait_for_data_update()`
...
- Проверка через `wait_for_data_update()` с retry
...
| ISR кэш: данные не обновляются | Next.js кэширование | Используй `wait_for_data_update()` |
...
- [ ] После модерации используется `wait_for_data_update()` с retry?
...
**Изменения:** Полный рефакторинг документации с детальным описанием архитектуры, паттернов, cleanup системы, модерации и инвалидации кэша.
