...
| TEST-010 | Галерея фотографий на странице гида/компании | ✅ Работает | `test_entity_photo_gallery.py::test_entity_photo_gallery` |
...
│   └── test_entity_photo_gallery.py # TEST-010
│
...
**Параметризация:** guide/company

**Сценарий:**
1. Создаёт 3 отзыва с 5 фотографиями каждый (15 фото total)
2. Переходит на страницу гида/компании
3. Проверяет галерею: 6 миниатюр + оверлей "+9" на последней
4. Открывает lightbox, проверяет счётчик "1 / 15"
5. Навигация: вперёд (2, 3, 4, 5) и назад (4)
6. Закрывает lightbox
7. Удаляет все отзывы через admin API
8. Проверяет исчезновение галереи

**Важно:**
- Использует существующие сущности:
  - Guide: "Георгий Челидзе" (ID: 1)
  - Company: "Georgian Adventures" (ID: 1) 
- Admin креды: `Philippe_testeroni` / `KeklikG0nnaKek!` (заглавная P)
- Галерея показывает макс. 6 фото, остальные через оверлей
- **Cleanup:** `clean_test_review` + teardown обрабатывает 404 если тест удалил сам
